...
data/
├── README.md                       # 本文件
├── manifest.json                   # B 文档元数据清单（doc_id/title/category/doc_type/bindings/file/state）
├── queries.json                    # B 期望查询集（query + 过滤条件 + 期望命中/排除/关键词）
...
│   ├── manual_reflow_oven_eq001.md     # 回流焊炉维修手册（ASSET_BOUND，故障码 E001/E002）
│   ├── manual_pickplace_eq002.md       # 贴片机维护手册（ASSET_BOUND，故障码 E101/E102）
│   ├── standard_ipc_a610.md            # IPC-A-610 焊点验收标准（GENERAL）
│   └── standard_esd.md                 # ESD 防静电规范（GENERAL）
└── trace/
...
| Neo4j 图检索器（A） | `FakeGraphRetriever` | 按 seed 返回 mock `TraceSubgraph`，`route_version` 过滤 Method 快照节点 |
