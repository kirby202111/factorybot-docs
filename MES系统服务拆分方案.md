...
| 服务 | 职责范围 | 核心用户 | 变更频率 |
|------|---------|---------|---------|
| **主数据服务** | 基础台账管理 + 工艺路线管理 | 计划部、质量部、工艺部、设备部 | 台账低频 / 工艺中高频 |
...
管理 MES 系统中所有基础性、配置性数据，是生产执行的数据基座。服务内按领域分两层，兼顾统一治理与独立演进。
...
│   ├── 设备台账（设备信息、线体、工位绑定）
│   ├── 人员台账（人员资质、技能矩阵、班次）
│   └── 物料台账（物料主数据、BOM 结构、替代料）
│
└── 工艺管理域
    ├── 工艺路线定义（工序组合、路由图）
    ├── 工序模板（27 种标准工序类型、参数范围）
    ├── 版本管理（草稿 / 审批中 / 生效 / 归档）
    └── 质量门禁配置（门禁规则、检验方法、升级规则）
```
...
- **点检超期锁定**：点检任务超过规定时间未执行，设备状态自动标记为"点检超期"，关联工位不可用过点
- **保养前冻结**：设备进入保养状态时，关联工位的过点校验自动拦截
- **状态变更事件**：点检/保养导致的设备状态变更，通过事件通知生产执行服务

### 2.5 设备状态管理

设备状态是连接设备管理服务与生产执行服务的关键桥梁：
...
**生产执行服务只看设备状态（可用/不可用），不关心是因为点检超期、故障还是保养中导致不可用。**
...
| 工艺缓存 | 订阅主数据服务的工艺版本事件，维护本地工艺路线缓存 |

### 3.3 过点校验流程

过点是生产执行服务的核心场景，涉及与主数据服务、设备采集服务的协作：
...
**缓存降级策略**：本地缓存未命中时，实时查询主数据服务作为降级路径，避免因缓存延迟导致误拦截。
...
|--------|---------|---------|------|---------|
| 生产执行服务 | 主数据服务 | 事件订阅（异步） | 工艺版本同步 | 秒级 |
| 生产执行服务 | 主数据服务 | REST 查询（同步） | 缓存未命中降级查询 | ≤500ms |
| 生产执行服务 | 设备管理服务 | REST 查询（同步） | 过点时查询设备实时数据 | ≤200ms |
| 生产执行服务 | 设备管理服务 | 事件订阅（异步） | 设备状态变更同步 | 秒级 |
| 主数据服务 | 生产执行服务 | 无 | 单向事件发布 | — |
| 设备管理服务 | 主数据服务 | 无 | 无直接依赖 | — |
...
| 场景 | 主导服务 | 协作服务 | 协作方式 |
|------|---------|---------|---------|
| 工艺变更生效 | 主数据服务 | 生产执行服务 | 发布事件 → 刷新工艺缓存 |
| 过点校验 | 生产执行服务 | 主数据服务（缓存） | 读本地缓存，降级查远程 |
...
| 质量门禁拦截 | 生产执行服务 | 主数据服务 | 读取门禁规则（缓存） |
| 返修再入点判定 | 生产执行服务 | 主数据服务 | 读取返修规则与再入点配置 |

---

## 五、未来拆分预留

主数据服务内部的两个域（基础台账域、工艺管理域）当前合并在一个服务内，但具备独立拆分条件。当出现以下任一情况时，应将工艺管理域拆为独立服务：
...
| DDD 领域 | 归属服务 | 说明 |
|---------|---------|------|
| 制造实体域 | 主数据服务（基础台账域） | 工厂/车间/产线/工位层次结构 |
| 工序域 | 主数据服务（工艺管理域） | 工序类型定义与参数模板 |
| 工艺路线域 | 主数据服务（工艺管理域） | 路线定义、版本管理、路由图 |
| 物料域 | 主数据服务（基础台账域） | 物料主数据、BOM、替代料 |
| 质量域 | 主数据服务（工艺管理域） | 质量门禁配置与规则定义 |
