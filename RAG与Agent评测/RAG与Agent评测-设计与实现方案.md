# RAG 与 Agent 评测 -- 设计与实现方案（Python 技术栈，覆盖 RAG 五路线 + Agent L1/L2/L3）

> 本文是 [RAG服务引入路线.md](../RAG服务/RAG服务引入路线.md) 五路线、[AGENT服务引入路线.md](../AGENT服务/AGENT服务引入路线.md) L1/L2/L3 与 [Agent可观测性-设计与实现方案.md](../AGENT服务/Agent可观测性-设计与实现方案.md) §9/§10/§11 的**收敛展开**，输出**评测对象分层、金标准数据集构建、指标体系、离线/在线评测流水线、LLM-as-judge、置信度标定、漂移检测、人工闭环、数据模型、包结构与代码骨架、实现步骤与约束落地**。
...
- **覆盖**：`rag-service`（五路线）与 `agent-service`（L1/L2/L3）的 AI 输出质量评测--检索质量、生成质量、Agent 行为、安全红线、置信度标定、漂移。
...
| **口径纪律** | 评测是设计阶段规划，不是"已上线全自动评测" | 讲"评测体系设计 / 取舍"，不说"我们已有 CI 全自动拦截发版"；🔴 未定阈值标注交还 |
...
### 2.1 评测对象总览（对齐 RAG 五路线 + Agent 三层级）
...
| 数据型 RAG（路线 C） | [数据型 RAG-详细设计.md](../RAG服务/数据型 RAG/数据型 RAG-详细设计.md) | Text2SQL | SQL 正确性 / 只读约束 / 语义层命中 | §5.3 |
| 追溯型 RAG（路线 A） | [追溯型 RAG-详细设计.md](../RAG服务/追溯型 RAG/追溯型 RAG-详细设计.md) | GraphRAG + 5M1E 综合 | 证据召回 / 根因准确 / 版本快照 | §5.4 |
| 防错即时辅助 RAG（路线 D） | [防错即时辅助 RAG-详细设计.md](../RAG服务/防错即时辅助 RAG/防错即时辅助 RAG-详细设计.md) | 事件驱动 + 预计算缓存 | 缓存命中 / 推送延迟 / 卡片准确 | §5.5 |
...
- **共享契约 `mes-eval`**：RAG 与 Agent 两个 Python 服务共用同一套评测契约（用例 schema、指标定义、judge 接口、结果表），抽成独立库 `mes-eval`。避免两套服务各写一套评测、口径不可比。🔴 `mes-eval` 的物理形态（独立 PyPI 包 vs monorepo 子模块）待定，倾向 monorepo 子模块降低跨仓库同步成本。
...
| 来源 | 价值 | 获取方式 | 占比目标 🔴 |
|------|------|---------|-----------|
| **转人工反馈（HANDOFF）** | ★★★★★ 最贴近线上分布 | 可观测 §9.4 转人工报告 -> 工程师确认/驳回 -> 自动落候选用例（§11） | 建议 ≥50% |
| **历史真实案例（HISTORY）** | ★★★★ 真实但需脱敏 | 从历史不良 / 8D 报告 / 返工单脱敏抽取 | 建议 ~30% |
| **人工构造边界（SYNTHETIC）** | ★★★ 覆盖长尾 / 安全红线 | 人工构造失效工艺 / 越权 / 证据缺失等边界 | 建议 ~20% |
...
- 🔴 各来源占比目标与总量（如 200 case 起步、覆盖 5M1E 各维度 + 14 上下文）待分批积累后定，不硬拍。
...
- 🔴 评测集是否可跨租户共享（多车间共用一个不良案例库）待合规确认；默认按租户隔离，跨租户用例显式标注 `cross_tenant=True` 且只验"拒绝访问"。
...
- 🔴 用例评审流程（谁有权新增 / 修改 / 废弃用例）待定，建议 AI 负责人 + 工艺/质量工程师双签。
...
指标按"通用 / 路线特有 / 层级特有 / 安全"四类组织。通用指标所有对象共用，特有指标按对象分节。所有指标在 `MetricsCalculator` 集中实现，不散落在各 runner。

### 5.2 文档型 RAG（路线 B）指标

| 指标 | 计算 | 目标 🔴 |
|------|------|--------|
| 文档召回率（context recall） | 检索到的相关文档 / 应检索文档 | 待定 |
| 引用命中率（citation hit） | 答案引用能对应到真实文档片段的占比 | 待定 |
| 版本过滤正确率 | 检索结果中 `status=ACTIVATED` 文档占比（应 100%） | 100%（硬） |
| 失效文档泄漏率 | 答案引用 `DEPRECATED` 文档的次数 / 总引用 | 0（硬失败） |
| 忠实度（faithfulness） | 答案 claims 能被检索文档支撑的占比（Ragas） | 待定 |
| 答案相关性（answer relevancy） | 答案与问题的相关度（Ragas） | 待定 |
...
| 指标 | 计算 | 目标 🔰 |
|------|------|--------|
| SQL 可执行率 | 生成的 SQL 能否成功执行（只读账号） | 待定 |
| 结果正确率 | SQL 结果集 == 预期结果集 | 待定 |
| 语义层命中率 | SQL 是否命中语义层视图（非原始表） | 待定 |
| 表白名单违反率 | SQL 访问非白名单表的次数 / 总 | 0（硬失败） |
| 写操作率 | SQL 含 INSERT/UPDATE/DELETE 的次数 / 总 | 0（硬失败） |
| 拒答率（该拒时拒） | 应拒答场景（如越权表）实际拒答占比 | 待定 |
...
| 指标 | 计算 | 目标 🔴 |
|------|------|--------|
| 证据召回（evidence recall） | 实际命中的预期上下文数 / 预期上下文数 | 待定 |
| 证据精度（evidence precision） | 命中预期上下文数 / 实际查的上下文数 | 待定 |
| 子图节点完整率 | 命中预期 node_id 数 / 预期 node_id 数 | 待定 |
| 根因 top-1 准确率 | top-1 假设命中预期根因类别 / 总 case | 待定 |
| 根因 top-3 准确率 | top-3 假设含预期类别 / 总 case | 待定 |
| 版本快照正确率 | 证据引用 route_version == 锚点版本 / 总 | 100%（硬） |
| 实体幻觉率 | 答案出现不存在的 node_id/SN/batch 次数 / 总 | 越低越好（硬失败非 0） |
| 降级查询 as_of 携带率 | 降级查询带 as_of 的次数 / 总降级 | 100%（硬） |
| 拒答正确率 | 应拒答场景实际拒答 + 转人工 / 应拒答 | 待定 |

> 追溯型 RAG 是护城河，指标最多。`版本快照正确率 = 100%` 是硬卡：证据引用的工艺版本必须等于过点当时锁定的 `routeVersion`（[追溯型 RAG-详细设计.md](../RAG服务/追溯型 RAG/追溯型 RAG-详细设计.md) §4.4 INV-CX-02），取"当前生效版"即判失败。`实体幻觉率`硬失败非 0--LLM 编造不存在的 node_id 在 MES 里是不可接受的（工程师按假证据处置会出事）。

### 5.5 防错即时辅助 RAG（路线 D）指标

| 指标 | 计算 | 目标 🔰 |
|------|------|--------|
| 缓存命中率 | 拦截命中预计算缓存 / 总拦截 | 待定（高） |
| 推送延迟 P99 | 拦截到卡片推送的 P99 延迟 | ≤200ms（硬，靠预计算） |
| 卡片准确率 | 推送的原因 + SOP 片段与实际匹配 / 总推送 | 待定 |
...
| 指标 | 计算 | 目标 🔴 |
|------|------|--------|
| 路由准确率 | 路由到的路线/工具 == 预期 / 总 | 待定 |
| 工具选择合理性 | 选对工具 / 总工具选择 | 待定 |
| 多余工具调用率 | 冗余工具调用 / 总 | 越低越好 |
| 收口正确率 | 该收口时收口 / 总 | 待定 |
...
| 指标 | 计算 | 目标 🔴 |
|------|------|--------|
| 根因 top-1/top-3 准确率 | 同追溯型 | 待定 |
| 工具序列合理性 | 实际工具序列符合预期模式 / 总 | 待定 |
| 证据充分度 | 实际引用证据数 / 该场景期望证据数 | 待定 |
| 步数冗余 | 实际步数 / 最少必要步数 | 越接近 1 越好 |
| route_version 携带率 | 工艺类工具调用带 route_version / 总工艺类调用 | 100%（硬） |
| 递归上限命中率 | 命中 recursion_limit / 总会话 | <2% |
| 证据为空率 | hypothesis 无 evidence / 总 | 0（硬失败） |
| 拒答正确率 | 应拒答时转人工 / 应拒答 | 待定 |
...
| 指标 | 计算 | 目标 🔴 |
|------|------|--------|
| 草稿采纳率 | 被工程师采纳下达的草稿 / 总草稿 | 待定 |
| 草稿合规率 | 输出为 intent+draft、未触发落库 / 总 | 100%（硬） |
| 草稿字段完整率 | 含 reentry_point/source_work_order_id 等必填 / 总 | 待定 |
...
| 指标 | 计算 | 目标 🔴 |
|------|------|--------|
| 编排正确率 | 编排步骤序列符合预期 / 总 | 待定 |
| gate 决策正确率 | gate APPROVED/REJECTED 符合预期 / 总 | 待定 |
| LLM 调用占比（顺利时） | 顺利换线中 LLM 调用数 / 总节点数 | 趋近 0（健康信号） |
...
class EvalRunner:
    ...
    async def _run_one(self, case: EvalCase, mv: str, pv: str) -> EvalResult:
        ...
        return EvalResult(
            ...
            ran_at=case.as_of,   # 🔴 时间戳由调用方注入，runner 内不取 now
        )
```
...
- **deterministic 尽量**：温度设 0（或最低）、随机种子固定；judge 模型也设低温度。完全 deterministic 在 LLM 下做不到，但同一 `prompt_version` 多次跑的方差要可接受（🔴 方差阈值待定）。
- 🔴 评测是否进 CI 硬门禁、门禁强度（block merge vs warn）待定；建议安全用例 hard block、质量用例 regression warn（§6.5）。
...
class CIGate:
    ...
    def decide(self, report: EvalReport, baseline: EvalReport) -> GateDecision:
        ...
        if report.accuracy < baseline.accuracy * 0.95:   # 🔴 阈值待定
            return GateDecision.BLOCK_QUALITY, "准确率回归超阈值"
        if report.ece > 0.1:                              # 🔴 阈值待定
...
- 🔴 hard/soft 的具体阈值与阻断 vs 告警的策略待评测数据积累后定，初始版安全 hard block + 质量 warn。
...
- 🔴 影子比例（如 5%）、采样维度（按租户 / 按对象类型均匀）、candidate 实例规格待定。
...
class CalibrationService:
    ...
    def fit_calibration(
        self, results: list[EvalResult], method: str = "platt"   # 🔴 method 待选
    ) -> CalibrationCurve:
...
- 🔴 标定方法（Platt vs isotonic）待评测数据规模确定后选：小样本用 isotonic 易过拟合，倾向 Platt；数据量大后可切 isotonic。
- **标定曲线版本化**：`calibration_version` 随模型 / 提示词变更重标，可观测侧记下用了哪版曲线--保证"置信度阈值决策可回溯"。
- **三源融合的标定**：可观测 §9.1 的三源（模型自评 / 证据充分度 / 工具成功率）融合权重 🔴 待标定数据拟合后定，初始版仅用模型自评 + 阈值。
...
- **自评偏见**：同模型当 judge 评自己 -> judge 用与被测不同的模型族（🔴 judge 模型选型待定，建议用最强模型当 judge，与被测模型解耦）。
- **版本盲区**：judge 可能不懂 MES 版本语义 -> rubric 显式告知锚点版本，judge 只判"是否等于锚点"，不自行判断"哪个版本对"。

### 10.4 judge 与人工对齐（强制）

- judge 上线当裁判前，必须先用一批**人工标注**的 case 验证一致性：judge 判定与人工判定的 agreement（Cohen's kappa / 一致率）≥ 🔴 阈值（如 0.8）才允许当裁判。
...
- 🔴 标注归属（哪条产线 / 哪个角色负责哪类 case）与激励待管理确认。
...
- 🔴 评测数据保留期限：`eval_run` / `eval_result_detail` 长期保留用于版本对比；`shadow_diff` 热数据 30 天 + 冷归档；`judge_result` 随 `eval_result_detail` 保留。待合规确认。
...
17. 扩充金标准集至覆盖 14 限界上下文 + 5M1E 各维度（🔴 规模待定）。
...
**Q：RAG 五路线和 Agent 三层级都用一套评测，会不会牵强？**
...
"RAG 与 Agent 评测是覆盖五路线 + L1/L2/L3 的只读旁路底座，用版本锚定的金标准集把'版本一致性 / 安全红线'从口头约束变成可断言的硬规则（失效工艺泄漏、写越界、过点侵入等零容忍 hard gate 阻断发版），用 LLM-as-judge + 人工闭环判生成质量，用标定曲线让置信度报得准、用漂移检测盯线上分布变化；它本身不进过点主事务、不旁路写、受租户隔离与脱敏约束，与 MES 防错理念同构--所有 AI 变更可度量、可回归、可拦下，宁可让评测拦下不合格的变更，也不盲发一条可能批量不良的答案。"
