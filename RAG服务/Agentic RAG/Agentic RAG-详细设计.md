...
[AGENT服务引入路线.md](../../AGENT服务/AGENT服务引入路线.md) §2.1 明确：**L0 收口型问答 Agent = RAG 路线 E**。E 不是又一个新能力，而是**统一入口收口层**--一个 Agent 入口，按问题路由到 RAG 路线 A/B/C/D 的工具 + L1/L2/L3 子 Agent（[RAG服务引入路线.md](../RAG服务引入路线.md) §2.5）。

- **价值在"收口"不在"新能力"**：A/B/C/D 是散落的检索/推送能力，L1/L2/L3 是专精 Agent。用户不该在 6 个入口间选择该问谁--E 做意图路由，把问题送到对的地方。
- **何时做**：等 A-D + L1-L3 成型后再收口（[RAG服务引入路线.md](../RAG服务引入路线.md) §3、[AGENT服务引入路线.md](../../AGENT服务/AGENT服务引入路线.md) §5）。否则 E 没工具可调，退化为套壳问答。**E 是最后一步，不是第一步**。

### 1.2 目标

把 A/B/C/D 的检索能力 + L1/L2/L3 的 Agent 能力收口成**一个统一入口**，让用户用自然语言提问，E 自动判断"这问题该用哪条路线答"，路由到对应工具或子 Agent，返回统一格式的答案 + 来源引用 + 工具链。
...
3. **数据查询类**："昨天 SMT1 号线 OEE" -> E 识别为结构化数据 -> 调 C 的 `query_data` 工具 -> 返回图表数据
4. **追溯事实类**："SN-001 过了哪几站" -> E 识别为追溯检索 -> 调 A 的 `query_traceability_graph` 工具 -> 返回子图
5. **草稿生成类**："给这批不良草拟返工单" -> E 识别为写意图 -> **委托 L2 子 Agent**（草稿不落库）-> 返回返工单草稿供工程师确认

### 1.3 硬边界（一开口就要讲）

| 边界 | 说明 | 落地 |
|------|------|------|
| **收口不造新能力** | E 只做路由 + 轻量组合 + 委托，不重建检索/推理/草稿能力 | E 无自有向量库/图谱/SQL 引擎，全部调 A/B/C/D + L1/L2/L3 |
| **只读（继承 L1）** | E 全程只读，无写工具；写意图委托 L2 但 L2 也只草拟不落库 | `ReadOnlyToolGate` 启动断言；写工具不注册到 E 的 ToolRegistry |
| **不进过点主事务** | E 不调过点引擎放行/拦截 API；过点辅助是 D 的拦后异步推送，E 不参与 | 过点 toolset 不暴露 `pass/judge` 类工具（继承 L1 §1.2） |
...
### 1.5 与 A/B/C/D RAG 的关系

| RAG 路线 | E 如何使用 | 形态 |
|---------|-----------|------|
| **A 追溯型** | 封装为 `query_traceability_graph` 工具，E 直接调（简单追溯）或委托 L1（深度诊断） | 工具 + 子 Agent |
| **B 文档型** | 封装为 `search_docs` 工具，E 直接调 | 工具 |
| **C 数据型** | 封装为 `query_data` 工具，E 直接调 | 工具 |
| **D 防错即时辅助** | E **不直接调**（D 是过点拦截事件驱动的推送通道，非问答工具）；E 可查历史拦截卡片作为辅助证据 | 独立通道（不纳入 E 工具集） |

- **D 为何不纳入 E 工具集**：D 是"过点拦截事件触发 -> 主动推工位屏幕"的推送形态（[防错即时辅助 RAG-详细设计.md](../防错即时辅助%20RAG/防错即时辅助%20RAG-详细设计.md) §1.3），不是"用户问 -> 答"的问答形态。E 是问答入口，与 D 的触发时机、受众都不同。两者独立，E 不调 D。
- **E 可查历史拦截卡片**：若用户问"昨天 AOI 站拦了哪些"，E 可调一个只读工具查 D 服务落库的拦截历史（`intercept_history`），作为辅助证据--这是只读查询，不是触发 D 推送。

### 1.6 与 Java 技术栈的关系

- E 服务用 Python，**不替换** MES 三大服务的 Java/Spring 栈--只通过 REST 调 A/B/C RAG 服务 + L1/L2/L3 Agent 服务的只读接口。
...
- E 是纯编排层，复用 A/B/C/D + L1/L2/L3 的既有契约，不造新契约。
...
| HTTP 客户端 | **httpx**（异步） | 调 A/B/C RAG + L1/L2/L3 Agent 只读 REST |
...
- A/B/C/D 已建好检索能力（向量/图谱/SQL/推送），L1/L2/L3 已建好 Agent 能力（诊断/草稿/编排）。E 重建等于重复造轮子 + 多份事实源。
...
- E 依赖 A/B/C/D + L1/L2/L3 服务的可达性：下游服务故障时 E 降级（§10.3），不硬答。
...
│  │ NL -> 意图/路由 │    │ A/B/C 工具调用   │    │ 委托 L1/L2 子Agent│ │
│  └──────────────┘    └────────┬─────────┘    └──────┬───────┘    │
│                               │                     │            │
│                       ┌───────▼─────────┐           │            │
│                       │ ToolRegistry    │           │            │
...
└─────────────────────────────────────────────────────┼────────────┘
        ...
        ▼             ▼             ▼                 ▼        ▼
  ┌──────────┐  ┌──────────┐  ┌──────────┐    ┌──────────┐ ┌──────────┐
  │ A 追溯RAG │  │ B 文档RAG │  │ C 数据RAG │    │ L1 诊断   │ │ L2 草稿   │
  │ /rag/trace│  │ /rag/docs │  │ /rag/data │    │ /agent/   │ │ /agent/   │
  └──────────┘  └──────────┘  └──────────┘    │ diagnose  │ │ draft    │
                                              └──────────┘ └──────────┘
```
...
- **工具与委托分离**：`ToolExecutor`（调 A/B/C 工具，单次或轻量组合）与 `SubAgentDelegator`（委托 L1/L2 子 Agent）解耦。简单查询走工具直答，深度场景走委托--单一职责（SRP）。
- **统一输出**：所有路径（工具直答/委托子 Agent）都收敛到 `AgentAnswer` 统一格式，用户看到一致体验。
- **trace 从 E 发起**：E 是入口，`trace_id` 从 E 生成，透传到 L1/L2/A/B/C，全链路可回溯。
...
意图严格对齐 A/B/C/D + L1/L2 的能力边界，不另造分类：
...
| `DATA_QUERY`（数据查询） | C 工具 `query_data` | 工具直答 | "昨天 OEE""返修 TOP3 缺陷" |
| `DRAFT_REQUEST`（草稿生成） | L2 子 Agent | 委托 | "给这批不良草拟返工单""草拟 8D 报告" |
| `INTERCEPT_HISTORY`（拦截历史） | 只读工具查 D 的 `intercept_history` | 工具直答 | "昨天 AOI 站拦了哪些" |
...
IntentRouter.classify(question)
   ...
   │     └─ 含"拦了哪些/拦截历史" -> INTERCEPT_HISTORY -> 只读工具
   │
   ├─ 2. LLM 兜底分类（规则命中不了）
...
| 意图 | 单步还是多步 | 路由 |
|------|------------|------|
| TRACE_FACT / DOC_LOOKUP / DATA_QUERY / INTERCEPT_HISTORY | 单步（一次工具调用即答） | `ToolExecutor` 直答 |
...
### 5.1 工具注册（对齐 A/B/C，只读白名单）

E 的 `ToolRegistry` 注册 A/B/C 的检索工具 + 拦截历史只读工具，全部只读：
...
| `query_intercept_history` | D 的历史（只读） | station_id, time_range, tenant_scope | - |

```python
class ToolDescriptor(BaseModel):
    name: str
    description: str
    route: str                        # "A" / "B" / "C" / "D-history"
...
[router_node] 意图分类
      │
      ├─ 单步意图 ──▶ [tool_node] 调 A/B/C 工具 ──▶ [converge_node] 收口
...
class AnswerSource(BaseModel):
    """答案来源引用（可回溯）。"""
    source_type: str          # "trace_node" / "sop_doc" / "sql" / "l1_hypothesis" / "intercept"
    ref: str                  # "node_id=CheckpointRecord:xxx" / "SOP:WELD-014@v3" / "audit_id=..."
    route: str                # "A" / "B" / "C" / "L1" / "L2" / "D-history"
...
4. **不进过点主事务闸**：过点 toolset 不暴露 `pass/judge`，过点辅助是 D 的拦后异步推送，E 不参与。
  ...
  app/
    ...
    infrastructure/
      ai/
        ...
        llm_factory.py
      acl/                     # 调 A/B/C RAG + L1/L2 Agent
        trace_rag.py           # A
        doc_rag.py             # B
        data_rag.py            # C
        intercept_history.py   # D 历史只读
...
- `infrastructure/acl/` 是防腐层，调 A/B/C + L1/L2，外部 DTO 经 Mapper 转内部视图。
...
class IntentRouter:
    ...
    RULES = [
        ...
        (["拦了哪些", "拦截历史"], IntentCategory.INTERCEPT_HISTORY),
    ]
...
class ToolExecutor:
    ...
    def _select_tool(self, intent: IntentCategory) -> str:
        return {
            ...
            IntentCategory.INTERCEPT_HISTORY: "query_intercept_history",
        }.get(intent, "")
```
...
class DependencyUnreachableGate(Exception):
    """启动时发现 A/B/C/D + L1/L2 依赖不可达，拒绝启动（E 是收口，依赖必须就绪）。"""
...
async def lifespan(app: FastAPI):
    ...
    # 2. 依赖可达性断言（E 收口的前提：A/B/C + L1/L2 必须可达）
    await assert_dependencies_reachable(app.state.acl_clients)
    yield

async def assert_dependencies_reachable(clients: dict) -> None:
    """E 是收口入口，下游 A/B/C + L1/L2 必须可达，否则退化为套壳。"""
    for name, client in clients.items():
        if not await client.ping():
            raise DependencyUnreachableGate(f"依赖不可达: {name}（E 收口要求 A-D + L1-L3 成型）")
```

- `assert_dependencies_reachable` 体现"E 是收口，依赖必须就绪"的判断--E 不该在 A/B/C + L1/L2 没就绪时启动，否则就是套壳问答。
...
- `trace_id` 从 E 生成，透传到 L1/L2/A/B/C（`traceparent` header）。
- `AnswerAudit` 记录问题/意图/路由/工具链/答案/`trace_id`，工程师可从答案回溯到每条路线的每步工具调用。

### 10.3 兜底

- **路由失败兜底**：`UNKNOWN` 意图 -> 转人工"该问题暂不支持"。
- **下游不可达兜底**：A/B/C/L1/L2 故障 -> 降级为"该路线暂时不可用，请联系工程师"或切换备选路线（如 A 故障用 C 查数据兜底）。
...
5. 实现 `ToolExecutor` + `ToolRegistry`（A/B/C 工具封装）（§9.3）。
6. 实现 ACL 客户端（A/B/C RAG + D 历史只读）（§7.2）。
...
> **前提**：E 的实现必须在 A/B/C/D + L1/L2 成型后启动（[RAG服务引入路线.md](../RAG服务引入路线.md) §3、[AGENT服务引入路线.md](../../AGENT服务/AGENT服务引入路线.md) §5）。E 是收口，不是起步。

---

## 12. 约束落地检查清单

- [ ] E 无自有检索/推理能力，全部调 A/B/C + 委托 L1/L2（§1.3 收口不造新能力）。
- [ ] 所有注册工具 `read_only=True`，`ReadOnlyToolGate` 启动断言生效（继承 L1）。
- [ ] `assert_dependencies_reachable` 启动断言：A/B/C + L1/L2 不可达时拒绝启动（E 收口前提）。
...
- [ ] `trace_id` 从 E 发起，透传到 L1/L2/A/B/C，全链路可回溯。
- [ ] 路由失败/下游不可达/低置信度/委托超时 -> 转人工兜底，不硬答。
- [ ] 答案带 `disclaimer`：辅助信息，最终处置需工程师确认。
- [ ] E 在 A/B/C/D + L1/L2 成型后启动，不提前做（避免套壳）。

---

## 13. 面试防守 Q&A

**Q：Agentic RAG 和你已经规划的 A/B/C/D + L1/L2/L3 是什么关系？是不是重复了？**
A：不重复，E 是收口。A/B/C/D 是散落的检索/推送能力，L1/L2/L3 是专精 Agent。用户不该在 6 个入口间选择该问谁--E 做统一入口 + 意图路由，把问题送到对的地方。E 不重建检索/推理能力，全部调 A/B/C 工具 + 委托 L1/L2 子 Agent。[AGENT服务引入路线.md](../../AGENT服务/AGENT服务引入路线.md) §2.1 明确 E = L0 收口型。E 是最后一步，不是第一步--等 A-D + L1-L3 成型后收口，否则没工具可调，退化为套壳问答。
...
**Q：E 怎么判断一个问题该走哪条路线？**
A：意图路由是 E 的命脉，规则优先 + LLM 兜底。高频问题关键词命中直定意图（"根因/5M1E"->L1，"OEE/TOP"->C，"怎么处置/SOP"->B，"草拟返工单"->L2），命中不了才走 LLM 结构化输出分类（`IntentCategory` 固化为 Enum）。路由错了全盘皆错，所以路由准确率是 E 的核心健康度指标。UNKNOWN 意图不硬答，转人工。

**Q：E 为什么不直接调 D（防错即时辅助）？**
A：因为 D 不是问答形态。D 是"过点拦截事件触发 -> 主动推工位屏幕"的推送形态（[防错即时辅助 RAG-详细设计.md](../防错即时辅助%20RAG/防错即时辅助%20RAG-详细设计.md) §1.3），触发时机（过点拦截后）和受众（操作工工位屏幕）都与 E（用户主动问答）不同。E 是问答入口，D 是推送通道，两者独立。但 E 可查 D 落库的拦截历史（只读工具 `query_intercept_history`），答"昨天 AOI 站拦了哪些"--这是只读查询，不是触发 D 推送。

**Q：E 会不会拖慢过点？**
A：不会。E 不调过点引擎放行/拦截 API，过点 toolset 不暴露 `pass/judge` 类工具（继承 L1 §1.2）。过点辅助是 D 的拦后异步推送，E 不参与。过点 P99 ≤200ms 仍由规则引擎保证，E 是管理层/工程师侧的问答入口，与过点执行完全无关。

**Q：E 路由错了怎么办？比如把数据查询误路由到 L1 诊断。**
...
**Q：上线了吗？**
A：这是设计阶段规划，不是已落地，且 E 是五条路线里最后做的。重点是四条架构判断：① E = L0 收口型，不造新能力，全部调 A/B/C + 委托 L1/L2；② 意图路由是核心，规则优先 + LLM 兜底；③ 广深分工--E 轻量组合（`recursion_limit=6`），深度诊断委托 L1，不重复；④ 继承全部只读红线（只读闸/版本闸/权限闸/不进过点主事务闸）。E 必须在 A-D + L1-L3 成型后启动，否则退化为套壳。诚实 + 体现架构判断力，比硬吹"已上线 Agentic RAG"得分高。

---

## 14. 一句话定位

"Agentic RAG 是五条 RAG 路线的收口--一个统一入口按问题意图路由到 A/B/C/D 工具 + 委托 L1/L2 子 Agent。E 不造新能力，全部调 A/B/C 检索 + 委托 L1 深度诊断/L2 草稿；意图路由规则优先 + LLM 兜底，把问题送到对的能力；广深分工--E 只做轻量组合（`recursion_limit=6`），跨上下文深度多步委托 L1，不重复；继承全部只读红线（`ReadOnlyToolGate`/版本/权限/不进过点主事务），所有路径收敛到带 `route_taken`+`tool_chain`+`sources` 的统一答案，trace 从 E 发起串联全链路。E 是最后一步，等 A-D + L1-L3 成型后收口，否则退化为套壳问答。"
