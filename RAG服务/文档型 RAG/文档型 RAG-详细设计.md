...
**Q：会不会拖慢过点？**
A：不会进过点主事务。过点 P99 ≤200ms（[领域总览.md](../../领域模型/领域总览.md) §4.1）是硬约束，文档摄入 / 重索引是异步消费事件，与过点判定完全解耦。文档检索容忍秒级延迟（[RAG服务引入路线.md](../RAG服务引入路线.md) §4）--它是事后 / 旁边问答工具，不是实时过点判定。即时辅助（路线 D）才需 <200ms，那靠预计算 + 缓存，不是现场跑文档检索。
...
**Q：操作工看不懂 LLM 长答案怎么办？**
A：工位场景要"一句话原因 + 一个动作 + 一个引用"，不是长文。系统提示词约束 LLM 输出精简结构化答案 + 强制引用 chunk；低置信度不硬答，转人工 / 转规则引擎--和 MES 防错理念一致，宁可拦下让人判（§6.4）。即时辅助（路线 D）进一步把高频拦截的"原因 + SOP 片段"预计算缓存，命中即推短卡片。
