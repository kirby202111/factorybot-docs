...
三层防线：
- 诊断 ReadOnlyToolGate：注册期 + 启动期双重断言，非只读工具拒绝注册。
- 草稿 NoWriteClientGate：见 domain/gate.py（扫描 ACL client 方法名禁写动词）。
- 编排 WriteToolGate：写工具必须声明 requires_confirmation + writes_via；supervisor 不持工具。
"""
...
class WriteToolGate(ToolGateError):
    """编排 写工具未声明 requires_confirmation / writes_via。"""
...
class ToolDescriptor(BaseModel):
    ...
    capability: str = "supervisor"  # supervisor | root_cause | fault_impact | traceability | draft_* | diagnosis
    read_only: bool = True
...
class ToolRegistry:
    ...
    def __init__(self, *, level: str = "diagnosis") -> None:
        # level: "diagnosis" 注册期拒绝写工具；"orchestration" 允许写工具但强制声明
        self._level = level
        self._descriptors: dict[str, ToolDescriptor] = {}

    # ---- 注册 ----
    def register(self, d: ToolDescriptor) -> None:
        if self._level == "diagnosis":
            # 诊断 注册期断言：非只读工具直接拒绝
            if not d.read_only:
                raise ReadOnlyToolGate(f"诊断 Agent 禁止注册非只读工具: {d.name}")
        else:
            # 编排：写工具必须声明 requires_confirmation + writes_via
            if not d.read_only:
    ...
    def validate_on_startup(self, *, supervisor_capability: str = "supervisor") -> None:
        ...
        - 诊断：再次确认全部只读。
        - 编排：写工具声明完整；supervisor 不持工具；禁止放行/拦截类工具；capability 互斥。
        """
        forbidden_prefixes = ("pass_judge", "force_release", "release_", "block_", "intercept_")
        for d in self._descriptors.values():
            if self._level == "diagnosis" and not d.read_only:
