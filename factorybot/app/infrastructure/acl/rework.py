...
class ReworkAclClient(BaseAclClient):
    ...
    async def query_repair_history(
        ...
        return await self._get_view(
            RepairHistoryView, f"/api/repair-history/{serial_no}", tenant=tenant,
    ...
    async def query_rework_orders(
        ...
        return await self._get_view(
            ReworkOrderListView, f"/api/work-orders/{wo_id}/rework-orders", tenant=tenant,
...
class ReworkWriteAclClient(BaseAclClient):
    ...
    async def issue_isolation(
        ...
        await self._validate_confirmation(confirmation, "issue_isolation")
        # 调应用服务 REST（header 带 X-Confirmation-Token）
    ...
    async def create_repair_order(
        ...
        await self._validate_confirmation(confirmation, "create_repair")
