...
3. **`msg_id` 去重表保留期与去重存储选型**：去重表保留期需 ≥ 最长补传窗口 + 安全余量（建议 ≥7 天），按 `received_ts` 定期清理。去重存储 v1 主推 DB `msg_id` 唯一索引 + 消费端幂等（v1 量级 DB 余量约百倍）；布隆过滤器为 DB 扛不住时的预案（持续 TPS 1000~10000 评估启用），预案启用时需定期重建或用计数布隆避免无限增长。
...
| 平台 `msg_id` 去重存储方案 | DB 唯一（v1 起步）/ Redis 布隆+DB（预案）/ Redis SET+异步 | v1 主推 DB 唯一 + 消费端幂等；布隆为 DB 扛不住时预案 | 模式 E |
