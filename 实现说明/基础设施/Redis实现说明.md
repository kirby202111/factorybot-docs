...
| 重难点 | 风险 | 应对 |
|---|---|---|
| **假阳性** | 布隆过滤器误判未处理过的 msg_id 为"已存在"，导致多查 DB | ① 假阳性回退 DB 唯一索引精确确认（§4.2）；② 误判率定为 **0.1%**，按预期 msg_id 总量算最优 hash 函数数与位数 |
...
| **保留期与补传窗口对齐** | 去重表保留期 < 最长补传窗口，补传的重复 msg_id 被当成新消息 | 保留期 **≥ 最长补传窗口 + 安全余量**（定为 **≥7 天**，与死信保留窗口对齐，[高频数据方案 §6.1](../高频数据/MES高频数据方案.md)） |
...
- 容量预估：单线百条/s × 7 天 ≈ 6 千万 msg_id/线，按车间线数 × 误判率 0.1% 算位图大小。本设计 ≤10 线满载 ≈ 6 亿 msg_id，位图 ≤1.1GB（10 个 hash 函数）。
- 膨胀治理（**选定滚动按天分桶**）：按天建 `mes:dedup:dc:{YYYY-MM-DD}`，保留 7 个桶（对齐 7 天保留期），滚动删除最旧桶。简单可靠，无需计数布隆模块。
...
- **maxmemory + 淘汰策略**：热层/缓存用 `allkeys-lru`；去重布隆不淘汰（按天滚动分桶治理）。
- `maxmemory` = **4GB**（2026-07-28 拍板，较估算 ~1.2GB 留宽余量应对线数/点位增长）。估算明细：去重布隆 ≤1.1GB（10 线满载 × 0.1% 误判）+ 热层 <10MB + 过点缓存 <100MB ≈ 1.2GB。v1 起步，上线后按 `used_memory` 实测量迭代。
...
| 布隆误判率 / 容量 | **0.1% / ≤1.1GB（10 线满载）** | 10 个 hash 函数；按线数 × 速率 × 7 天算（§4） |
| 布隆膨胀治理 | **滚动按天分桶** | 保留 7 桶滚动删最旧（§4.5） |
