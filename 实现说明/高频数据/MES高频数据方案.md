...
| 方案 | 说明 | 适用 |
|---|---|---|
| **DB 唯一索引** | `msg_id` 唯一索引，INSERT 冲突即判重；+ 消费端 msg_id 幂等两层防御 | **v1 起步主推**；v1 量级（10线×10设备×1Hz≈100 TPS）DB 余量约百倍 |
| Redis 布隆过滤器 + DB 唯一索引 | 布隆快速判重（少量假阳性回退 DB），DB `msg_id` 唯一索引兜底 | **DB 扛不住时的预案**；持续 TPS 1000~10000 评估启用，>10000 必要 |
| Redis SET + 异步落 DB | Redis 判重快，DB 异步持久化 | 需处理 Redis 与 DB 一致窗口 |

🔴 **去重表保留期**需 ≥ 最长补传窗口 + 安全余量（建议 ≥ 7 天，与死信保留窗口对齐），按 `received_ts` 定期清理超期 msg_id。布隆过滤器（预案启用时）需定期重建或用计数布隆避免无限增长。
...
| **平台去重** | 补传排空 / 突发到达 | 去重表写放大（布隆预案启用时叠加膨胀） | 去重吞吐上限 + 背压到网关 |
...
| 平台去重存储方案 | DB 唯一（v1 起步）/ Redis 布隆+DB（预案）/ Redis SET+异步 DB | v1 主推 DB 唯一 + 消费端幂等；布隆为 DB 扛不住时预案（TPS 1000~10000 评估） |
